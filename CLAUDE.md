...
npm run screenshot                                                         # Screenshot localhost:3000 → temporary screenshots/
node screenshot.mjs http://localhost:3000 hero-v2                          # Screenshot with custom label
node tools/generate_image.js "prompt" --aspect 16:9 -o media/hero-bg.png   # Generate image via OpenRouter
```

## Architecture Notes

- **ESM-only** — `package.json` has `"type": "module"`. All scripts use ES module `import` syntax.
- **No build step** — default output is a single `index.html` with inline styles (Tailwind via CDN).
- **Dev server** (`serve.mjs`) — vanilla Node.js HTTP static file server on port 3000. No hot reload.
- **Screenshot tool** (`screenshot.mjs`) — Puppeteer full-page capture at 1440x900 viewport, 1.5s animation settle delay. Auto-increments filenames in `temporary screenshots/`.
- **Image generation** (`tools/generate_image.js`) — calls OpenRouter API. Reads `.env` manually (no dotenv dependency). Requires `OPENROUTER_API_KEY`.
